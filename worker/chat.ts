...
export class ChatHandler {
  ...
  constructor(aiGatewayUrl: string, apiKey: string, model: string, env: Env, sessionId: string, systemPrompt?: string) {
    ...
    this.systemPrompt = systemPrompt || 'You are a helpful AI assistant that helps users build and deploy web applications. You provide clear, concise guidance on development, deployment, and troubleshooting. Keep responses practical and actionable.';
  }
  /**
    ...
    return Promise.all(
      openAiToolCalls.map(async (tc) => {
        try {
          ...
          const result = await executeTool(tc.function.name, args, this.env, this.sessionId); // Pass env and sessionId
          return {
            id: tc.id,
  ...
  /**
   * Update the system prompt for this chat handler
   */
