...
export type ToolResult = WeatherResult | { content: string | object } | ErrorResult | { imageUrls?: string[], message?: string, generatedImageUrl?: string };
...
const customTools = [
  ...
  },
  {
    ...
    function: {
      name: 'list_r2_images',
      description: 'Lists all image URLs stored in the R2 bucket for the current session.',
  ...
  },
  {
    type: 'function' as const,
    function: {
      name: 'delete_r2_image',
      description: 'Deletes an image from the R2 bucket using its filename.',
      parameters: {
        type: 'object',
        properties: { filename: { type: 'string', description: 'The filename of the image to delete (e.g., "my-image.jpg")' } },
        required: ['filename']
      }
    }
  },
  {
    ...
    function: {
      ...
      description: 'Generates a new image based on a text prompt using the Qwen-Image model. Especially good for creating images with Chinese text.',
      parameters: {
        type: 'object',
        properties: {
          prompt: { type: 'string', description: 'A detailed description of the image to generate, including any text to be rendered.' }
        },
        required: ['prompt']
      }
    }
  },
  {
    type: 'function' as const,
    function: {
      name: 'qwen_image_edit_text',
      description: 'Edits the text within an existing image using the Qwen-Image-Edit model. Maintains original font, size, and style.',
      parameters: {
        type: 'object',
        properties: {
          image_url: { type: 'string', description: 'The URL of the image to edit from the media library.' },
          text_to_replace: { type: 'string', description: 'The original text in the image that needs to be replaced.' },
          new_text: { type: 'string', description: 'The new text to be rendered in the image.' }
        },
        required: ['image_url', 'text_to_replace', 'new_text']
      }
    }
  }
];
...
export async function executeTool(name: string, args: Record<string, unknown>, env?: Env, sessionId?: string): Promise<ToolResult> {
  try {
    switch (name) {
      case 'get_weather':
        return {
      ...
      case 'list_r2_images': {
        if (!env || !sessionId) return { error: 'R2 environment or session ID not available.' };
        const controller = getAppController(env);
        const imageUrls = await controller.listR2Images(sessionId);
        return { imageUrls, message: `Found ${imageUrls.length} images in your media library.` };
      }
      case 'delete_r2_image': {
        if (!env || !sessionId) return { error: 'R2 environment or session ID not available.' };
        const { filename } = args;
        if (typeof filename !== 'string') return { error: 'Filename is required for deletion.' };
        const controller = getAppController(env);
        const success = await controller.deleteR2Image(sessionId, filename);
        return { message: success ? `Image '${filename}' deleted successfully.` : `Failed to delete image '${filename}'.` };
      }
      case 'qwen_image_generate':
      case 'qwen_image_edit_text': {
